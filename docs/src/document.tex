...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{\mono{extra\_limma\_vars} (\mono{null} OR nested \mono{str}}: If \mono{null}, nothing happens. If nested \mono{str}, each string in the nested list is treated similar to the \mono{experimental\_design} string, adding an additional variable to the \mono{limma} analysis. This can be useful to control for additional confounding variables in the experiment. For an example, see the default configuration file.
